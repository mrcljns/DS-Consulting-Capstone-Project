...
if __name__ == '__main__':
    ...
    feedback_df['Customer Remarks'] = feedback_df['Customer Remarks'].apply(lambda x: remove_keywords(remove_digits(x), keyword_set))
    ...
    word_freq_df = pd.DataFrame(frequencies.toarray(), columns=vectorizer_model.get_feature_names_out(), index=feedback_df['Unique id'])

    # Reset the index to make 'Agent_name' a column
    word_freq_df = word_freq_df.reset_index()

    # Unpivot the DataFrame from wide to long format
    word_freq_long_df = pd.melt(word_freq_df, id_vars=['Unique id'], var_name='word', value_name='frequency')
    ...
    word_freq_long_df.to_csv('../data/word_frequencies_long.csv', index=False)